...
def calculate_ratios(vertices):
    ratios = []
    for perm in permutations(['A', 'B', 'C', 'D']):
        ratio = getattr(getattr(vertices, perm[2]), 'x') / getattr(getattr(vertices, perm[2]), 'y') - getattr(getattr(vertices, perm[0]), 'x') / getattr(getattr(vertices, perm[0]), 'y')
        ratios.append(ratio)
    return ratios
...
if vertex is not None:
    ...
    ratios = calculate_ratios(vertex)
    print("Ratios for all permutations:")
    for i, ratio in enumerate(ratios):
        print(f"Permutation {i+1}: {ratio}")
