...
if image is not None and vertex is not None:
    ...
    # Add white circle at point O
    intensity = 255
    radius = 400
    white_circle = create_white_circle(image.shape, (light_source_x, light_source_y), radius, intensity)
    image_with_circle = cv2.addWeighted(image_with_line, 1, white_circle, 0.7, 0)

    # Display the image with the drawn line and white circle
    cv2.imshow('Image with Line and Circle', image_with_circle)
    cv2.waitKey(0)
