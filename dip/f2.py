...
if image is not None and vertex is not None:
    ...
    mask = np.zeros_like(image_with_line)
    roi_points = np.array([extended_point1, extended_point2, (extended_point2[0], 0), (extended_point1[0], 0)])
    cv2.fillPoly(mask, [roi_points], (255, 255, 255))

    # Make everything inside the circle have an intensity of -255 for the color
    image_with_line = cv2.addWeighted(image_with_line, 1, mask, -255, 0)
