...
if image is not None and vertex is not None:
    ...
    # Draw a line from O towards the first vertex
    cv2.line(image_with_line, (int(vertex_o_x), int(vertex_o_y)),
             (int(max_ratio_vertices.vertex1.x), int(max_ratio_vertices.vertex1.y)), (0, 255, 0), 2)

    # Draw a line from O towards the second vertex
    cv2.line(image_with_line, (int(vertex_o_x), int(vertex_o_y)),
             (int(max_ratio_vertices.vertex2.x), int(max_ratio_vertices.vertex2.y)), (0, 255, 0), 2)
